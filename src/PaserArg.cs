/*
            ...
            string output = null;
            OptionSet optionparse = new OptionSet()
               .Add<string>("k=|key=", "Loacl State Path", v => localstate = v)
               .Add<string>("c=|cookie=", "Browser Cookie Path", v => cookies = v)
               .Add<string>("o=|output=", "File Output Path", v => output = v)
               .Add<string>("l=|login=", "Browser Login Data Path", v => logindata = v)
               .Add<string>("history=", "Browser history Path", v => history = v)
               .Add<string>("bookmarks=", "Browser bookmarks Path", v =>bookmarks= v)
               .Add("h|help", "Get help", v => help = v != null);
            try
            {
                optionparse.Parse(args);
                if (help)
                {
                    outputHelp(optionparse);
                }
                else if (!string.IsNullOrEmpty(localstate) && !string.IsNullOrEmpty(cookies))
                {
                    if (!string.IsNullOrEmpty(output))
                    {
                        Browser_cookies(localstate, cookies, output);
                    }
                    else {
                        Browser_cookies(localstate, cookies);
                    }
